...
Review all changes between $ARGUMENTS (default: main) and HEAD.

Run `git diff ${ARGUMENTS:-main}...HEAD` to get the diff. Then for each changed file:
...
4. **Tests** — run `git diff ${ARGUMENTS:-main}...HEAD --name-only` and flag any source file that has no corresponding test file
